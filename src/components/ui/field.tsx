...
function FieldTitle({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      data-slot="field-label"
      className={cn('flex w-fit items-center gap-2 font-medium font-mono text-sm leading-snug group-data-[disabled=true]/field:opacity-50', className)}
      {...props}
    />
  );
}
...
function FieldError({
  ...
  const content = useMemo(() => {
    ...
    return <ul className="ml-4 flex list-disc flex-col gap-1">{errors.map((e) => e?.message && <li key={e.message}>{e.message}</li>)}</ul>;
  }, [children, errors]);
