/**
 * Captura V3 — getUserMedia + circle overlay SVG
 * Baseado no componente 14 do develop
 * Config: 768x768 JPEG 0.85, facingMode toggle, SEM circle crop (quadrado)
 * Diferente do V1/V2: captura quadrado sem recorte circular
 */
...
function CaptureV3Dialog({ open, onClose, onCapture }: { open: boolean; onClose: () => void; onCapture: (_img: string) => void }) {
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="flex! h-[85vh] max-w-lg flex-col gap-0 overflow-hidden border-none bg-black p-0" showCloseButton={false}>
        <DialogHeader className="flex shrink-0 flex-row items-center justify-between bg-black/60 px-3 py-2 text-white backdrop-blur-md">
          <DialogTitle className="text-white">V3 — Square 768 JPEG</DialogTitle>
          <DialogDescription className="sr-only">Captura quadrada 768x768 JPEG sem circle crop</DialogDescription>
        ...
        <div className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden bg-black">
          ...
          <div className="pointer-events-none absolute inset-0 z-10">
            <svg width="100%" height="100%" className="absolute inset-0" role="img">
              ...
              <defs>
                <mask id="circle-mask-v3">
                  <rect width="100%" height="100%" fill="white" />
                  <circle cx="50%" cy="45%" r="35%" fill="black" />
                </mask>
              </defs>
              <rect width="100%" height="100%" fill="rgba(0,0,0,0.6)" mask="url(#circle-mask-v3)" />
