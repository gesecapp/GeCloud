/**
 * Captura V6 — getUserMedia + rect overlay SVG (3:4)
 * Baseado no componente 15 do develop
 * Config: 1920x1080 JPEG 0.85, toggle câmera, rect overlay, captura full frame sem crop
 */
...
function CaptureV6Dialog({ open, onClose, onCapture }: { open: boolean; onClose: () => void; onCapture: (_img: string) => void }) {
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="flex! h-[85vh] max-w-lg flex-col gap-0 overflow-hidden border-none bg-black p-0" showCloseButton={false}>
        <DialogHeader className="flex shrink-0 flex-row items-center justify-between bg-black/60 px-3 py-2 text-white backdrop-blur-md">
          <DialogTitle className="text-white">V6 — Rect FullFrame 1920 JPEG</DialogTitle>
          <DialogDescription className="sr-only">Captura retangular full frame 1920x1080 JPEG</DialogDescription>
        ...
        <div className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden bg-black">
          ...
          <div className="pointer-events-none absolute inset-0 z-10">
            <svg width="100%" height="100%" className="absolute inset-0" role="img">
              <title>Guia retangular</title>
              <defs>
                <mask id="rect-mask-v6">
                  <rect width="100%" height="100%" fill="white" />
                  <rect x="10%" y="15%" width="80%" height="65%" rx="12" fill="black" />
                </mask>
              </defs>
              <rect width="100%" height="100%" fill="rgba(0,0,0,0.6)" mask="url(#rect-mask-v6)" />
